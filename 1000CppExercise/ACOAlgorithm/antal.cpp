...
struct antType {
	...
	int tabu[MAX_CITIES];//luu danh sach cac thanh pho da tham
	int path[MAX_CITIES];
	double tourLength;
};
...
void init()
{
	...
	for (ant = 0; ant < MAX_ANTS; ant++)
	{
		if (to == MAX_CITIES)
			to = 0;
		//Đặt mỗi con kiến ở một thành phố, 
		//trong trường hợp số kiến nhiều hơn số thành phố thì sẽ có thành phố được đặt nhiều hơn 1 con
		ants[ant].curCity = to++;

		for (from = 0; from < MAX_CITIES; from++)
		{
			ants[ant].tabu[from] = 0;	//dùng giá trị 0 cho biết thành phố đó con kiến chưa thăm đến, 
										//dùng giá trị 1 để cho biết con kiến đã thăm qua thành phố đó
			ants[ant].path[from] = -1;
		}

		ants[ant].pathIndex = 1;		//pathIndex lưu số city mà con kiến đã đi qua.
		...
		ants[ant].tabu[ants[ant].curCity] = 1;//ta bu bằng 1 hiểu là đã đến thành phố đó
...
void updateTrails()
{
	...
	for (from = 0; from<MAX_CITIES; from++)
	{
		for (to = 0; to<MAX_CITIES; to++)
		{
			if (from != to)
			{
				phero[from][to] *= (1.0 - RHO); //tính lượng pheromone còn lại sau bay hơi
	...
	for (ant = 0; ant<MAX_ANTS; ant++)
	{
		for (i = 0; i<MAX_CITIES; i++)
		{
			...
			phero[from][to] += (QVAL / ants[ant].tourLength);//lượng thêm vào
